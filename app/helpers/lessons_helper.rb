module LessonsHelper
  ...
  def higher_control(sortable, options)
    button_to("&uarr;", options[:url] + "?id=#{sortable.id}&direction=up", :class => "up_button") unless sortable.first?
  end
  
  def lower_control(sortable, options)
    button_to("&darr;", options[:url] + "?id=#{sortable.id}&direction=down", :class => "down_button") unless sortable.last?
  end
  
  def sortable_list(sortable_array, options)
    sortable_array.inject([]) do |list_item_array, page|
      list_item_array << "<li class=\"sortable\"><div class=\"sorting_controls\">#{sorting_controls(page, options)}</div>#{link_to(page.title, lesson_page_path(page.lesson,page))}</li>"
      list_item_array
    end.join
  end
